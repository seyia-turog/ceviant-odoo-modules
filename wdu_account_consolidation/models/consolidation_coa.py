...
class ConsolidationChart(models.Model):
    ...
    def action_open_mapping(self):
        ...
        return {
            ...
            'view_mode': 'tree',
            'views': [[self.env.ref('wdu_account_consolidation.consolidation_account_tree_mapping').id, 'list']],
            'domain': [('chart_id', '=', self.id)],
            'context': {},
            'name': _('Account Mapping: %(chart)s', chart=self.name),
            'search_view_id': [self.env.ref('wdu_account_consolidation.consolidation_account_search_mapping').id, 'search']
        }
    ...
    def setting_consolidation_action(self):
        ...
        :return: the action to execute
        """
        action = self.env["ir.actions.actions"]._for_xml_id("wdu_account_consolidation.consolidation_chart_action_onboarding")
    ...
    def action_save_onboarding_consolidation_step(self):
        self.env['onboarding.onboarding.step'].action_validate_step(
            'wdu_account_consolidation.onboarding_onboarding_step_setup_consolidation'
        )
    ...
    def setting_consolidated_chart_of_accounts_action(self):
        ...
        :return: the action to execute
        """
        action = self.env["ir.actions.actions"]._for_xml_id("wdu_account_consolidation.consolidation_account_action")
        last_chart = self.search([], order="id desc", limit=1)
        action.update({
            'context': {'default_chart_id': last_chart.id, 'search_default_chart_id': last_chart.id},
            'views': [
                (self.env.ref('wdu_account_consolidation.consolidation_account_tree_onboarding').id, 'list'),
                (False, 'form')
            ]
        })
        self.env['onboarding.onboarding.step'].action_validate_step(
            'wdu_account_consolidation.onboarding_onboarding_step_setup_ccoa'
        )
        return action
    ...
    def setting_create_period_action(self):
        ...
        :return: the action to execute
        """
        action = self.env["ir.actions.actions"]._for_xml_id("wdu_account_consolidation.consolidation_period_action_onboarding")
        last_chart = self.search([], order="id desc", limit=1)
