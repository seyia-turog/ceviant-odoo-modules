...
class ConsolidationPeriod(models.Model):
    ...
    def action_open_mapping(self):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("wdu_account_consolidation.account_mapping_action")
        action.update({
            'domain': [('company_id', '=', company_id)],
            # dont know why but it's needed otherwise "cannot read type of undefined" js error
            'views': [[self.env.ref('wdu_account_consolidation.account_mapping_tree').id, 'list']],
            'context': {
    ...
    def action_open_form(self):
        ...
        """
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("wdu_account_consolidation.consolidation_period_action")
        action.update({
            'views': [[False, 'form']],
            'res_id': self.id
        })
    ...
    def action_open_trial_balance_grid(self):
        ...
        return {
            ...
            'search_view_id': [self.env.ref('wdu_account_consolidation.trial_balance_grid_search').id, 'search']
        }
    ...
    def action_save_onboarding_create_step(self):
        ...
        self.env['onboarding.onboarding.step'].action_validate_step(
            'wdu_account_consolidation.onboarding_onboarding_step_create_consolidation_period'
        )
