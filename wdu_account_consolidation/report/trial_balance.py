...
class TrialBalanceCustomHandler(models.AbstractModel):
    ...
    def _get_custom_display_config(self):
        return {
            ...
            'components': {
                'AccountReportFilters': 'wdu_account_consolidation.ConsolidationReportFilters',
                'AccountReportLineCell': 'wdu_account_consolidation.ConsolidationReportLineCell',
            },
            'templates': {
                'AccountReportHeader': 'wdu_account_consolidation.ConsolidationReportHeader',
            },
            'pdf_export': {
                'pdf_export_main_table_header': 'wdu_account_consolidation.pdf_export_main_table_header',
                'pdf_export_filters': 'wdu_account_consolidation.pdf_export_filters',
                'pdf_export_main_table_body': 'wdu_account_consolidation.pdf_export_main_table_body',
            },
        }
    ...
    def _get_journal_col(self, journal, options):
        ...
        if journal.currencies_are_different and journal.company_period_id:
            ...
            return {
                ...
                'template': 'wdu_account_consolidation.cell_template_consolidation_report',
            }
    ...
    def action_open_view_grid(self, options):
        ...
        return {
            ...
            'view_type': 'grid',
            'views': [
                [self.env.ref('wdu_account_consolidation.view_trial_balance_report_grid').id, 'grid'],
                [self.env.ref('wdu_account_consolidation.view_trial_balance_report_graph').id, 'graph'],
                [self.env.ref('wdu_account_consolidation.consolidation_journal_line_form').id, 'form']
            ],
            'context': {
    ...
    def action_open_audit(self, options, params=None):
        ...
        if len(journal_lines) == 0:
            return None
        action = self.env["ir.actions.actions"]._for_xml_id("wdu_account_consolidation.view_account_move_line_filter")
        action.update({
            'context': {
            ...
            'views': [(self.env.ref('wdu_account_consolidation.view_move_line_tree_grouped_general').id, 'list')]
        })
        return action
